...
class Client:
    ...
    def predict(self, text_list):
        sentences = [["search", sentence]for sentence in text_list]
        print(sentences[1])
        predict_examples = self.processor.create_examples(sentences, set_type='pred',file_base=False)
        ...
        print('beforte:%s'%type(next_element))
        ...
        print('after:%s'%type(next_element))
        ...
        output = [np.argmax(i) for i in result['output']]
        lable = [label_list[index] for index in output]

        return output, lable

if __name__ == '__main__':
    client = Client()
    # msg = ["麻烦给我16倍速度播放综艺忍者蝙蝠侠","大陆片老男人","把衣帽间窗帘ᐁ关到留条缝"]
    msg = ["大江大河44集", "把衣帽间窗帘ᐁ关到留条缝"]
    prediction,lable = client.predict(msg)
